...
import { APP_IDENTITY, getRpcEndpoint, getCluster, USE_DEVNET } from '../config/constants';
...
class ProgramService {
  ...
  async checkGenesisToken(walletPubkey) {
    // Devnet mode: always show as Genesis holder for demo purposes
    if (USE_DEVNET) {
  ...
  async _executeMwaTransaction(buildTxFn) {
    ...
    return transact(async (wallet) => {
      ...
      let userPubkey;
      try {
        const decoded = require('buffer').Buffer.from(authResult.accounts[0].address, 'base64');
        userPubkey = new PublicKey(decoded);
      } catch {
        userPubkey = new PublicKey(authResult.accounts[0].address);
      }
  ...
  async hashContent(content) {
    ...
    try {
      ...
      logger.error('[ProgramService] js-sha256 not available, using fallback hash');
